...
local cv_kd_knockdown_plyinveh = CreateConVar(cvPrefix .. "knockdown_playerinvehicle", 1, cvTags,
    "是否击倒在载具内的玩家(如果玩家在车里则试图让他们离开载具, 如果他们能)", 0, 1)

local cv_kd_knockdown_defaultbehaviour = CreateConVar(cvPrefix .. "knockdown_defaultbehaviour", 1, cvTags,
    "启用默认的击倒检测, 如果你有什么东西对击倒检测做了彻底改变可以关掉", 0, 1)
local cv_kd_knockdown_percentdamage_enabled = CreateConVar(cvPrefix .. "knockdown_percentdamage_enabled", 1, cvTags,
    "允许在伤害大于玩家最大生命百分比时击倒玩家", 0, 1)
local cv_kd_knockdown_percentdamage = CreateConVar(cvPrefix .. "knockdown_percentdamage", 0.5, cvTags,
    "击倒玩家所需的伤害百分比, 请注意 如果其它条件满足也是可以击倒的", 0, 1)
local cv_kd_knockdown_mindamage = CreateConVar(cvPrefix .. "knockdown_mindamage", 10, cvTags,
    "击倒玩家最小所需的伤害, 请注意 如果力度足够也是可以击倒的", 0)
local cv_kd_knockdown_mindamageforce = CreateConVar(cvPrefix .. "knockdown_mindamageforce", 2500, cvTags,
    "击倒玩家最小所需的伤害力度, 请注意 如果伤害足够也是可以击倒的", 0)
...
local cv_kd_damagecalc_usetakedamage = CreateConVar(cvPrefix .. "knockdown_usetakedamage", 0, cvTags,
    "使用TakeDamageInfo并更进一步修改BulletTable, 可能会出现没受到伤害且力度不够时仍被击倒的情况", 0, 1)

local cv_kd_ctrl_useheadang = CreateConVar(cvPrefix .. "control_useheadangles", 0, cvTags,
    "在玩家转动视角时使用玩家目前的头部朝向计算, 可能会导致无法舒适翻滚", 0, 1)
local cv_kd_ctrl_luacode_uselocaleyeangles = CreateConVar(cvPrefix .. "control_luacode_uselocaleyeangles", 1, cvTags,
    "[手感][代码相关] 将要设置的EyeAngles\"局部化\"(经过Roll旋转), 可以解决部分武器包的武器后坐力垂直于地面的问题, 但可能有其它奇怪的现象", 0, 1)
local cv_kd_ctrl_luacode_dontclampeyeang = CreateConVar(cvPrefix .. "control_luacode_dontclampeyeang", 1, cvTags,
    "[手感] 瞄准时不钳制玩家的视角(roll仍然会被锁住), 对部分eye设置的很史的模型有奇效, 同时在手感上也和ZCity相似", 0, 1)

local cv_kd_perf_luacode_nexttick = CreateConVar(cvPrefix .. "performance_luacode_nexttick", 0.01, cvTags,
    "[性能][代码相关] 下次统一运行控制器Tick()的时间, 这个值越大布娃娃效果越拉跨(但性能会好点我猜), 不建议大于0.03", 0)
local cv_kd_perf_luacode_ctrlamnt = CreateConVar(cvPrefix .. "performance_luacode_maxprocessedcontroller", 10, cvTags,
    "[性能][代码相关] 一tick间隔(或者是你设置的tick间隔)处理的控制器数量, 设置为0则无上限", 0)
local cv_kd_perf_luacode_tracelevel = CreateConVar(cvPrefix .. "performance_luacode_tracelevel", 4, cvTags,
    "[性能][代码相关] 查找Trace的层数, 越高越\"广泛\", 操作涉及到布娃娃的面越广, 但有潜在的性能消耗(低于4的话工具枪无法选择布娃娃)", 0)
local cv_kd_perf_luacode_deltamul = CreateConVar(cvPrefix .. "performance_luacode_deltamul", 1, cvTags,
    "[性能][代码相关][需重启] ShadowControl运算中delta的乘数, 把它调低对安装了VPhysics-Box3D的人有帮助 不要让它为0, 推荐值为0.5", 0)
...
CreateConVar(cvPrefix .. "rag_minmasslimit", 1, cvTags,
    "设置布娃娃特定部位的最小重量, 并降低布娃娃其它部位的重量. 对于物理部件特多的布娃娃可能会有Bug, 关掉这个会导致特定模型伸手可以飞天", 0, 1)
...
local clcv_ctrl_nodefkeybind = CreateClientConVar(cvPrefix .. "cl_control_disabledefaultkeybind", "0", true, true,
    "禁用默认的瞄准方法(按住E瞄准), 可能对某些服务器的自定义按键设置有帮助", 0, 1)
local clcv_ctrl_reversedaiming = CreateClientConVar(cvPrefix .. "cl_control_reversedaiming", "0", true, true,
    "[仅按住E可用时] 按住E取消瞄准 而不是进行瞄准", 0, 1)
local clcv_ctrl_aim = CreateClientConVar(cvPrefix .. "cl_control_autoaim", "0", true, true,
    "[仅自定义按键可用时] 击倒时默认开启瞄准(0: 关闭, 1: 仅主动击倒, 2: 任何情况下被击倒(需要服务器打开相关设置!))", 0, 2) -- ToDo: 把2加上
CreateClientConVar(cvPrefix .. "cl_control_altaimkey", "0", true, true, "[仅按住E可用时] 按住[慢走键](默认是LAlt)进行瞄准", 0, 1)
CreateClientConVar(cvPrefix .. "cl_getup_smoothtransitioninterval", "0.15", true, true,
    "在起身后视角在和老视角和实际视角的过渡时间, 总而言之就是能让起身的视角转换看上去丝滑一点(我相信你不会把它改成1以上的值)", 0)
...
--[[Savee_ARKD.FunctionHooks.Add("Entity.EyePos", "test1", function(self, ...)
...
Savee_ARKD.FunctionHooks.Add("Entity.EyePos", "test", function(self, ...)
...
function Savee_ARKD.entTypeCheck(ent)
...
function Savee_ARKD.getController(ent)
...
local mtx = Matrix()
function Savee_ARKD.getBoneMatrixPosAng(rag, name)
...
for _, str in ipairs(trs) do
    ...
    Savee_ARKD.FunctionHooks.Add("util." .. str, "Savee_AdvRagKnockdown_HitScanMod", trOverride)
    Savee_ARKD.FunctionHooks.AddPost("util." .. str, "Savee_AdvRagKnockdown_HitScanMod", trOverride2)
end
...
for _, str in ipairs(setItOnMe) do
    ...
    Savee_ARKD.FunctionHooks.Add("Entity.Set" .. str, "Savee_AdvRagKnockdown_Syncing", override)
end
...
local tickCo = coroutine.create(function()
    while true do
        ...
        for _, ent in ipairs(ctrls) do
            ...
            if not IsValid(ent:GetRagdoll()) or not IsValid(own) or own:IsMarkedForDeletion() or own:Health() <= 0 then
                ent:RemoveSelf()
                continue
            end
...
if SERVER then
    ...
    local function calcRagDamage(rag, di, take)
        ...
        -- 参见Savee_ARKD.FunctionHooks(修改Trace的那些)
    ...
    function Savee_AdvRagKnockdown_ReplaceRagConstraint(ctrl, rag, pObjs, bchild, bparent, minAng, maxAng, fric)
        ...
        local oldHandPos, oldHandAng, oldArmPos, oldArmAng = lHandP:GetPos(), lHandP:GetAngles(), lArmP:GetPos(),
            lArmP:GetAngles()
        ...
        if not parentMtx or not childMtx then
        ...
        local const = constraint.AdvBallsocket(rag, rag, lArm, lHand, wtlLH, nil, 0, 0, minAng.p, minAng.y, minAng.r,
            maxAng.p, maxAng.y, maxAng.r, fric, fric, fric, 0, 1)
    ...
    function Savee_AdvRagKnockdown_ReplaceRagConstraints(rag, pObjs)
        ...
        Savee_AdvRagKnockdown_ReplaceRagConstraint(ctrl, rag, pObjs, "ValveBiped.Bip01_L_Hand",
            "ValveBiped.Bip01_L_Forearm", -angMax, angMax, 0)
        Savee_AdvRagKnockdown_ReplaceRagConstraint(ctrl, rag, pObjs, "ValveBiped.Bip01_R_Hand",
            "ValveBiped.Bip01_R_Forearm", -angMax, angMax, 0)

        Savee_AdvRagKnockdown_ReplaceRagConstraint(ctrl, rag, pObjs, "ValveBiped.Bip01_Head1", "ValveBiped.Bip01_Spine2",
            -Angle(15, 60, 50), Angle(15, 40, 50), 0)

        angMax = Angle(80, 80, 80)
        Savee_AdvRagKnockdown_ReplaceRagConstraint(ctrl, rag, pObjs, "ValveBiped.Bip01_L_UpperArm",
            "ValveBiped.Bip01_Spine2", -angMax, angMax, 0)
        Savee_AdvRagKnockdown_ReplaceRagConstraint(ctrl, rag, pObjs, "ValveBiped.Bip01_R_UpperArm",
            "ValveBiped.Bip01_Spine2", -angMax, angMax, 0)
    end
    ...
    Savee_ARKD.FunctionHooks.Add("Player.AnimRestartGesture", "Savee_AdvRagKnockdown_Sync",
        function(ply, _, act, autokill, ...)
            local ctrl = getController(ply)
    ...
    --[[Savee_ARKD.FunctionHooks.Add("Entity.DispatchTraceAttack", "Savee_AdvRagKnockdown_BulletStuffs", function(ply, _, traceResult, ...)
    ...
    --[[Savee_ARKD.FunctionHooks.Add("Entity.SetModel", "Savee_AdvRagKnockdown_Sync", function(ent, ...)
    ...
    --[[Savee_ARKD.FunctionHooks.AddPost("Player.CreateRagdoll", "Savee_AdvRagKnockdown_InheritRagVel", function(ply, ...)
    ...
    -- @EzBodyDamage
    Savee_ARKD.FunctionHooks.Add("Entity.TakeDamageInfo", "Savee_AdvRagKnockdown_DmgModSupport",
        function(ent, di, raw, ...)
            --local ctrl = getController(ent)
            local dmg = di:GetDamage()
    ...
    Savee_ARKD.FunctionHooks.Add("Entity.DispatchTraceAttack", "Savee_AdvRagKnockdown_DmgModSupport",
        function(atk, di, res, raw, ...)
            --local ctrl = getController(ent)
            local ent = res.Entity
            local dmg = di:GetDamage()
            if raw or not IsValid(ent) or not cv_kd_damagecalc_usetakedamage:GetBool() then
                return __undetoured(atk, di, res,
                    raw, ...)
            end
            --print(di)
            if ent:IsRagdoll() then
    ...
    Savee_ARKD.FunctionHooks.Add("Entity.TakeDamage", "Savee_AdvRagKnockdown_DmgModSupport",
        function(ent, dmg, atk, inf, raw, ...)
            --local ctrl = getController(ent)
    ...
    Savee_ARKD.FunctionHooks.Add("Entity.BodyTarget", "Savee_AdvRagKnockdown_Sync", function(ply, vec, raw, ...)
        local ctrl = getController(ply)
    ...
    Savee_ARKD.FunctionHooks.Add("Entity.HeadTarget", "Savee_AdvRagKnockdown_Sync", function(ply, vec, raw, ...)
        local ctrl = getController(ply)
        if not IsValid(ctrl) then return __undetoured(ply, vec, raw, ...) end
        local rag = ctrl:GetRagdoll()
    ...
    Savee_ARKD.FunctionHooks.Add("NPC.Disposition", "Savee_AdvRagKnockdown_Sync", function(ply, ent, raw, ...)
        local ctrl = getController(ent)
    ...
    --[[Savee_ARKD.FunctionHooks.Add("_G.SetPhysConstraintSystem", "Savee_AdvRagKnockdown_Test", function(sys, ...)
        --print(sys)
    ...
    concommand.Add(cvPrefix .. "doknockdown", function()
        ...
        timer.Simple(isSP and 0 or lp:Ping() / 800, function()
            sendAimingMsg(1)
            oldAimingState = true
        end)
    end)
    ...
    hook.Add("StartCommand", "!Savee_AdvRagKnockdown_RagOperation", function(ply, cmd)
        ...
        local aimingBind = clcv_ctrl_nodefkeybind:GetBool() and input.LookupBinding("+advragknockdown_aimweapon") or
            input.LookupBinding(cvPrefix .. "toggleaimweapon")
    ...
    --[[Savee_ARKD.FunctionHooks.Add("Entity.SetBoneMatrix", "Savee_AdvRagKnockdown_TPIKSupport", function(ply, i, mtx, ...)
    ...
    Savee_ARKD.FunctionHooks.Add("Entity.SetBonePosition", "Savee_AdvRagKnockdown_TPIKSupport", function(ply, i, pos, ang, ...)
